...
def stylize(masked_img,
    ...
    # set up background layer weights
    layer_bg_weight = 1.0
    style_bg_layers_weights = {}
    for style_layer in BACKGROUND_LAYERS:
        style_bg_layers_weights[style_layer] = layer_bg_weight
        layer_bg_weight *= background_layer_weight_exp
        
    # normalize background layer weights
    bg_layer_weights_sum = 0
    for style_layer in BACKGROUND_LAYERS:
        bg_layer_weights_sum += style_bg_layers_weights[style_layer]
    for style_layer in BACKGROUND_LAYERS:
        style_bg_layers_weights[style_layer] /= bg_layer_weights_sum
    

    # compute content features in feedforward mode
    g = tf.Graph()
    with g.as_default(), g.device('/device:GPU:0'), tf.Session() as sess:
    ...
    with tf.Graph().as_default():
        ...
        masked_initial = initial * masked_img

        image = tf.Variable(initial)
        masked_image = tf.Variable(masked_initial)
        net, mask_net = vgg.net_preloaded_mask(vgg_weights, image, masked_image, pooling)
        ...
        # total loss
        loss = content_loss + style_loss + tv_loss + bg_loss
