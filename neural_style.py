...
def main():
    ...
    # set up style background blend weights  
...
def mask_imread(mask_path, content_path):
    
    content_image_mask = scipy.misc.imread(mask_path).astype(np.float)
    content_image = scipy.misc.imread(content_path).astype(np.float)
    
    # create the mask
    mask = np.where(content_image_mask >= 255, content_image_mask, 1)
